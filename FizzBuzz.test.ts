import { FizzBuzz } from "./FizzBuzz";

describe('FizzBuzz', () =>{
let fizzBuzz = new FizzBuzz();

    describe('count method', () =>{
        it("should return 1 when given 1", () =>{

            //Arrange

            //Act
            let actual = fizzBuzz.count(1);

            //Assert
            expect(actual).toBe("1");
        });

         it("should return 2 when given 2", () =>{
            
            let actual = fizzBuzz.count(2);

            expect(actual).toBe("2");
        });
        it("should return fizz when given 3", () =>{
            
            let actual = fizzBuzz.count(3);
            expect(actual).toBe("fizz");
        })

        it("should return 4 when given 4", () =>{

            let actual =  fizzBuzz.count(4);
            expect(actual).toBe("4");
        })

       it("should return buzz when given 5", ()=>{
        let actual = fizzBuzz.count(5);
        expect(actual).toBe("buzz");
       })

       it("should return fizz whgen given 6", () =>{
        let actual = fizzBuzz.count(6);
        expect(actual).toBe("fizz");
       })

       it("should return fizz when given 9", () =>{
        let actual = fizzBuzz.count(9);
        expect(actual).toBe("fizz");
       })

        it("should return fizz when given 10", () =>{
        let actual = fizzBuzz.count(10);
        expect(actual).toBe("buzz");
       })

       it("should return fizzbuzz when 15", ()=>{
        let actual = fizzBuzz.count(15)
        expect(actual).toBe("fizzbuzz");
       })
    });
});